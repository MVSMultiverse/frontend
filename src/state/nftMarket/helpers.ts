...
import { BASE_BAPI_GATEWAY_URL } from 'config'
import { getErc721CollectionContract } from 'utils/contractHelpers'
...
export const getCollectionsApi = async (): Promise<ApiCollection[]> => {
  try {
    if (GET_COLLECTIONS_API_PRODUCTION_DATA) {
      return GET_COLLECTIONS_API_PRODUCTION_DATA.data.content
    }
    console.error('Failed to fetch NFT collections')
    return []
  } catch (error) {
    return []
  }
...
export const getCollectionApi = async (collectionAddress: string): Promise<ApiCollection> => {
  return GET_COLLECTION_API_PRODUCTION_DATA.data
}
...
export const getNftApi = async (
  ...
  let _bunnyId = bunnyId || null
  const meta = FETCH_NFT_INFO_DEVELOPMENT_DATA.data.data
  if (!_bunnyId) {
    const contract = getErc721CollectionContract(null, collectionAddress)
    _bunnyId = await contract.getBunnyId(tokenId)
    const _data = meta.find(it => it.bunnyId === String(_bunnyId));
    const obj: ApiResponseSpecificToken['data'] = {
      ..._data,
      tokenId,
      image: JSON.parse(_data.metadataJSON),
      collection: {
        name: _data.collectionName
      }
    }
    return obj
  }
...
export const getNftsFromDifferentCollectionsApi = async (
  ...
  const collectionAddresses = [...new Set(from.map((it) => it.collectionAddress))]
  const collectionAddressWithTokenIds = new Map()

  const promises = collectionAddresses.map(async (collectionAddress) => {
    const tokenIds = from.filter((it) => it.collectionAddress === collectionAddress).map((it) => it.tokenId)
    if (tokenIds.length > 0) {
      const fetchApiData = async () => {
        try {
          const res = await fetch(`${BASE_BAPI_GATEWAY_URL('functions/accountAssociatedNftsBunnyIds')}&collectionAddress=${collectionAddress}&tokenIds=${tokenIds}`)
          if (res.ok) {
            const { result } = await res.json()
            const data = result.bunnyIds || []
            collectionAddressWithTokenIds.set(collectionAddress, tokenIds)
            return data
          }
          console.error(`API: Failed to fetch ${collectionAddress}'s bunnyIds:`, res.statusText)
          return []
        } catch (error) {
          return fetchApiData()
        }
  ...
  const NFT_LOCAL_DB_MAP = new Map()
  JSON.parse(localStorage.getItem('NFT_LOCAL_DB_V1') || '[]').map((localNft: any) =>
    NFT_LOCAL_DB_MAP.set(localNft.bunnyId, localNft),
  )

  const nfts = []
  for (let i = 0; i < responses.length; i++) {
    for (let j = 0; j < responses[i].length; j++) {
      const bunnyId = responses[i][j]
      const tokenId = collectionAddressWithTokenIds.get(collectionAddresses[i])[j]
      const localNft = NFT_LOCAL_DB_MAP.get(bunnyId)
      const nft = { ...localNft, tokenId }
      nfts.push(nft)
    }
  }
...
export const fetchWalletTokenIdsForCollectionsAddresss = async (
  account: string,
  collections: string,
): Promise<TokenIdWithCollectionAddress[]> => {

  const contract = getErc721CollectionContract(null, collections)
