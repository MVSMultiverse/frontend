...
import {
  ...
  fetchNftsFiltered,
  fetchWalletTokenIdsForCollectionsAddresss
} from './helpers'
import {
  State,
  Collection,
...
>('nft/fetchNftsFromCollections', async ({ collectionAddress }) => {
  if (collectionAddress === getMvsNftAddress()) {
  ...
  //   console.log('Object.values(nfts.data)', Object.values(nfts.data))
...
export const fetchUserNfts = createAsyncThunk<NftToken[], { account: string; collections: ApiCollections }>(
  'nft/fetchUserNfts',
  async ({ account, collections }) => {
...
export const NftMarket = createSlice({
  ...
  reducers: {
    ...
    setUserTokenIds: (state, action: PayloadAction<TokenIds[]>) => {
      state.data.user.tokenIds = [...state.data.user.tokenIds, ...action.payload]
    },
    removeUserTokenIds: (state, action: PayloadAction<TokenIds[]>) => {
      const copyOfState = [...state.data.user.tokenIds]
      for (let i = 0; i < action.payload.length; i++) {
        const nftToRemove = copyOfState.find((nft) => nft.tokenId === action.payload[i].tokenId && nft.collectionAddress === action.payload[i].collectionAddress)
        const indexInState = copyOfState.indexOf(nftToRemove)
        copyOfState.splice(indexInState, 1)
      }
      state.data.user.tokenIds = copyOfState
    },
  },
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchCollectionsTokenIds.fulfilled, (state, action) => {
      state.data.user.tokenIds = action.payload
      state.data.user.userNftsInitializationState = UserNftInitializationState.INITIALIZED
    })
    builder.addCase(fetchCollectionsTokenIds.rejected, (state) => {
      state.data.user.userNftsInitializationState = UserNftInitializationState.ERROR
    })
    builder.addCase(fetchCollectionsTokenIds.pending, (state) => {
      state.data.user.userNftsInitializationState = UserNftInitializationState.INITIALIZING
    })
  },
})
