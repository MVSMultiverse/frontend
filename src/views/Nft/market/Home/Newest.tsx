...
import { Heading, Flex, Grid, Button, ChevronRightIcon } from '@pancakeswap/uikit'
...
const useNewestNfts = () => {
  ...
  useEffect(() => {
    const fetchNewestNfts = async () => {
      ...
      const nfts = []
      for (let i = 0; i < nftsFromSg.length; i++) {
        const nftFromApi = nftsFromApi[i]
        if (nftFromApi) {
          nfts.push({ ...nftFromApi, marketData: nftsFromSg[i] })
        }
      }
      setNewestNfts(nfts)
    }
    ...
    <div>
      <Flex justifyContent="space-between" alignItems="center" mb="26px">
        ...
        <Button
          ...
          to={`${nftsBaseUrl}/collections/${getMvsNftAddress().toLowerCase()}`}
          variant="secondary"
          scale="sm"
