...
export const StyledModal = styled(Modal)<{ stage: HarvestingStage }>`
  padding: 0;
  & > div:last-child {
    padding: 0;
  }
  & h2:first-of-type {
    ${({ stage, theme }) =>
      stage === HarvestingStage.APPROVE_AND_CONFIRM || stage === HarvestingStage.CONFIRM
...
export const StyledCard = styled(Card)<{ selected: boolean }>`
  border: 1px solid ${({ selected, theme }) => (selected ? theme.colors.primary : '#fff')};
`
