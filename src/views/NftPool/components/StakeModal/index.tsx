...
import { useMvsNftContract, useMvsNftPoolContract } from 'hooks/useContract'
...
import { setUserPoolNfts } from 'state/nftPool/reducer'
...
const modalTitles = {
  [HarvestingStage.SELECTED_NFT]: 'Selected NFTs',
...
const StakeModal: React.FC<HarvestModalProps> = ({ userNfts, onDismiss }) => {
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onApprove: () => {
      return callWithGasPrice(mvsNftContract, 'setApprovalForAll', [mvsNftPoolContract.address, true])
    },
    ...
    onConfirm: () => {
      ...
      return callWithGasPrice(mvsNftPoolContract, 'batchStake', [ids])
    },
    onSuccess: async ({ receipt }) => {
      dispatch(setUserPoolNfts({ nfts: selectedNft, collectionAddress: address }))
      onDismiss()
